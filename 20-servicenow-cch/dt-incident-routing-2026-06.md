...
| # | Item | Status |
|--:|---|---|
| 1 | **Identify what sets `current.u_level = "Level 1"` at insert.** | Dictionary default ruled out — `u_level` has no default value (confirmed via `sys_dictionary` filter `Table = incident`). Setter is therefore one of: a UI Policy, a Client Script (onLoad), or a Business Rule (before-insert). Needs a live `sys_ui_policy` / `sys_script_client` / `sys_script` query against the incident table. |
